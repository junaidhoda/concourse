#!/usr/bin/env python3
"""
Suvarnabhumi Airport Restaurant Fetcher
---------------------------------------
Fetches every restaurant / dining venue at Suvarnabhumi Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  bkk_restaurants.json   full structured data
  bkk_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_bkk_restaurants.py
"""
...
SUVARNABHUMI_BBOX = (13.668, 100.718, 13.722, 100.782)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Main Terminal": (13.69, 100.7501),
    "Satellite Terminal SAT-1": (13.6952, 100.7527),
}
...
USER_AGENT   = "BKKDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "bkk_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "bkk_restaurants.csv")
...
_TERMINAL_RE = re.compile(
    r"\b(Main\s+Terminal|Satellite\s+Terminal(?:\s+SAT-?1)?)\b",
    re.IGNORECASE,
)
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        ...
        if m:
            ...
            label = ("Satellite Terminal SAT-1"
                     if "satellite" in text else "Main Terminal")
...
TERMINAL_ORDER = ['Main Terminal', 'Satellite Terminal SAT-1', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Suvarnabhumi Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Suvarnabhumi Airport dining venues from OpenStreetMap \u2026")

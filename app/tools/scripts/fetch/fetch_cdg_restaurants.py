#!/usr/bin/env python3
"""
Paris Charles de Gaulle Airport Restaurant Fetcher
--------------------------------------------------
Fetches every restaurant / dining venue at Paris Charles de Gaulle Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  cdg_restaurants.json   full structured data
  cdg_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_cdg_restaurants.py
"""
...
CDG_BBOX = (48.985, 2.5, 49.04, 2.6)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (49.0093, 2.5475),
    "Terminal 2A": (49.0053, 2.5623),
    "Terminal 2B": (49.0043, 2.5655),
    "Terminal 2C": (49.0069, 2.5608),
    "Terminal 2D": (49.0058, 2.564),
    "Terminal 2E": (49.0095, 2.5712),
    "Terminal 2F": (49.0083, 2.5748),
    "Terminal 2G": (49.0041, 2.5773),
    "Terminal 3": (49.0021, 2.5607),
}
...
USER_AGENT   = "CDGDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "cdg_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "cdg_restaurants.csv")
...
_TERMINAL_RE = re.compile(r"\bTerminal\s+(1|2[A-G]|3)\b", re.IGNORECASE)
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        ...
        if m:
            label = f"Terminal {m.group(1).upper()}"
...
TERMINAL_ORDER = ['Terminal 1', 'Terminal 2A', 'Terminal 2B', 'Terminal 2C', 'Terminal 2D', 'Terminal 2E', 'Terminal 2F', 'Terminal 2G', 'Terminal 3', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Paris Charles de Gaulle Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Paris Charles de Gaulle Airport dining venues from OpenStreetMap \u2026")

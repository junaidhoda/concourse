#!/usr/bin/env python3
"""
Dubai International Airport Restaurant Fetcher
----------------------------------------------
Fetches every restaurant / dining venue at Dubai International Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  dxb_restaurants.json   full structured data
  dxb_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_dxb_restaurants.py
"""
...
DXB_BBOX = (25.228, 55.320, 25.280, 55.420)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (25.2532, 55.3658),
    "Terminal 2": (25.2501, 55.3557),
    "Terminal 3": (25.2566, 55.3908),
}
...
USER_AGENT   = "DXBDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "dxb_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "dxb_restaurants.csv")
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Dubai International Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Dubai International Airport dining venues from OpenStreetMap \u2026")

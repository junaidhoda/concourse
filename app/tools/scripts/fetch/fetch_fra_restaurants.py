#!/usr/bin/env python3
"""
Frankfurt Airport Restaurant Fetcher
------------------------------------
Fetches every restaurant / dining venue at Frankfurt Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  fra_restaurants.json   full structured data
  fra_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_fra_restaurants.py
"""
...
FRA_BBOX = (50.018, 8.525, 50.06, 8.62)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1 Area A": (50.0379, 8.5622),
    "Terminal 1 Area B": (50.0387, 8.558),
    "Terminal 1 Area C": (50.0393, 8.5546),
    "Terminal 1 Area Z": (50.0363, 8.5521),
    "Terminal 2 Area D": (50.0326, 8.5706),
    "Terminal 2 Area E": (50.0342, 8.5736),
}
...
USER_AGENT   = "FRADiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "fra_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "fra_restaurants.csv")
...
_TERMINAL_RE = re.compile(
    r"\bTerminal\s+(\d)\s+(?:Area\s+)?([A-EZ])\b",
    re.IGNORECASE,
)
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        ...
        if m:
            label = f"Terminal {m.group(1)} Area {m.group(2).upper()}"
...
TERMINAL_ORDER = ['Terminal 1 Area A', 'Terminal 1 Area B', 'Terminal 1 Area C', 'Terminal 1 Area Z', 'Terminal 2 Area D', 'Terminal 2 Area E', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Frankfurt Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Frankfurt Airport dining venues from OpenStreetMap \u2026")

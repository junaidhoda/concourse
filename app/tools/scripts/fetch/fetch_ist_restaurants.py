#!/usr/bin/env python3
"""
Istanbul Airport Restaurant Fetcher
-----------------------------------
Fetches every restaurant / dining venue at Istanbul Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  ist_restaurants.json   full structured data
  ist_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_ist_restaurants.py
"""
...
IST_BBOX = (41.24, 28.69, 41.32, 28.8)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Main Terminal": (41.2617, 28.7436),
    "GAP Terminal": (41.2813, 28.7351),
}
...
USER_AGENT   = "ISTDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "ist_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "ist_restaurants.csv")
...
_TERMINAL_RE = re.compile(r"\b(Main|GAP)\s+Terminal\b", re.IGNORECASE)
_TERMINAL_NAME_MAP = {"main": "Main Terminal", "gap": "GAP Terminal"}
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        ...
        if m:
            label = _TERMINAL_NAME_MAP.get(m.group(1).lower())
            if label and label in TERMINALS:
                return label
    return None
...
TERMINAL_ORDER = ['Main Terminal', 'GAP Terminal', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Istanbul Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Istanbul Airport dining venues from OpenStreetMap \u2026")

#!/usr/bin/env python3
"""
John F. Kennedy International Airport Restaurant Fetcher
--------------------------------------------------------
Fetches every restaurant / dining venue at John F. Kennedy International Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  jfk_restaurants.json   full structured data
  jfk_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_jfk_restaurants.py
"""
...
JFK_BBOX = (40.61, -73.825, 40.66, -73.75)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (40.6399, -73.7814),
    "Terminal 4": (40.6426, -73.7802),
    "Terminal 5": (40.6447, -73.7853),
    "Terminal 7": (40.6463, -73.7897),
    "Terminal 8": (40.6441, -73.7947),
}
...
USER_AGENT   = "JFKDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "jfk_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "jfk_restaurants.csv")
...
TERMINAL_ORDER = ['Terminal 1', 'Terminal 4', 'Terminal 5', 'Terminal 7', 'Terminal 8', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  John F. Kennedy International Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching John F. Kennedy International Airport dining venues from OpenStreetMap \u2026")

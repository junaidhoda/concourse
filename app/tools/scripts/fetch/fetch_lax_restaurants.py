#!/usr/bin/env python3
"""
Los Angeles International Airport Restaurant Fetcher
----------------------------------------------------
Fetches every restaurant / dining venue at Los Angeles International Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  lax_restaurants.json   full structured data
  lax_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_lax_restaurants.py
"""
...
LAX_BBOX = (33.92, -118.43, 33.96, -118.37)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (33.9397, -118.4104),
    "Terminal 2": (33.9412, -118.4095),
    "Terminal 3": (33.9427, -118.4083),
    "Terminal 4": (33.9444, -118.4067),
    "Terminal 5": (33.9454, -118.4044),
    "Terminal 6": (33.9461, -118.4016),
    "Terminal 7": (33.9458, -118.3987),
    "Terminal 8": (33.9452, -118.3957),
    "Tom Bradley International": (33.9433, -118.3927),
}
...
USER_AGENT   = "LAXDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "lax_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "lax_restaurants.csv")
...
_TERMINAL_RE = re.compile(
    r"\bTerminal\s+(\d)\b|\b(Tom\s+Bradley(?:\s+International(?:\s+Terminal)?)?)\b",
    re.IGNORECASE,
)
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        m = _TERMINAL_RE.search(tags.get(fld, ""))
        if m:
            label = (f"Terminal {m.group(1)}" if m.group(1)
                     else "Tom Bradley International")
...
TERMINAL_ORDER = ['Terminal 1', 'Terminal 2', 'Terminal 3', 'Terminal 4', 'Terminal 5', 'Terminal 6', 'Terminal 7', 'Terminal 8', 'Tom Bradley International', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Los Angeles International Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Los Angeles International Airport dining venues from OpenStreetMap \u2026")

#!/usr/bin/env python3
"""
London Gatwick Airport Restaurant Fetcher
-----------------------------------------
Fetches every restaurant / dining venue at London Gatwick Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  lgw_restaurants.json   full structured data
  lgw_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_lgw_restaurants.py
"""
...
GATWICK_BBOX = (51.14, -0.21, 51.168, -0.15)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "North Terminal": (51.1602, -0.1783),
    "South Terminal": (51.1562, -0.163),
}
...
USER_AGENT   = "LGWDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "lgw_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "lgw_restaurants.csv")
...
_TERMINAL_RE = re.compile(r"\b(North|South)\s+Terminal\b", re.IGNORECASE)
...
def _terminal_from_tags(tags: dict) -> Optional[str]:
    for fld in ("addr:housename", "addr:street", "description", "name"):
        ...
        if m:
            label = f"{m.group(1).title()} Terminal"
...
TERMINAL_ORDER = ['North Terminal', 'South Terminal', 'Outside Airport']
...
def main() -> None:
    log.info("Fetching London Gatwick Airport dining venues from OpenStreetMap \u2026")

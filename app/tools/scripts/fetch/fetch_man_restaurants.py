#!/usr/bin/env python3
"""
Manchester Airport Restaurant Fetcher
-------------------------------------
Fetches every restaurant / dining venue at Manchester Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  man_restaurants.json   full structured data
  man_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_man_restaurants.py
"""
...
MANCHESTER_BBOX = (53.34, -2.31, 53.375, -2.25)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (53.3578, -2.2797),
    "Terminal 2": (53.3528, -2.2801),
    "Terminal 3": (53.3579, -2.2717),
}
...
USER_AGENT   = "MANDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "man_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "man_restaurants.csv")
...
TERMINAL_ORDER = ['Terminal 1', 'Terminal 2', 'Terminal 3', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Manchester Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Manchester Airport dining venues from OpenStreetMap \u2026")

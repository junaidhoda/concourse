#!/usr/bin/env python3
"""
Singapore Changi Airport Restaurant Fetcher
-------------------------------------------
Fetches every restaurant / dining venue at Singapore Changi Airport
using the Overpass API (OpenStreetMap).  100 % free - no API key required.

Outputs
  sin_restaurants.json   full structured data
  sin_restaurants.csv    flat spreadsheet-friendly format
  stdout                          human-readable summary

Usage
  pip install requests
  python fetch_sin_restaurants.py
"""
...
CHANGI_BBOX = (1.328, 103.955, 1.385, 104.01)   # (south, west, north, east)

# Approximate centre of each terminal building (derived from OSM data).
TERMINALS = {
    "Terminal 1": (1.3545, 103.9889),
    "Terminal 2": (1.3581, 103.9893),
    "Terminal 3": (1.3601, 103.9887),
    "Terminal 4": (1.3349, 103.9981),
}
...
USER_AGENT   = "SINDiningResearch/1.0 (educational)"

MAX_RETRIES  = 4
OUTPUT_JSON  = _os.path.join(JSONS_DIR, "sin_restaurants.json")
OUTPUT_CSV   = _os.path.join(RESTAURANTS_DIR, "sin_restaurants.csv")
...
TERMINAL_ORDER = ['Terminal 1', 'Terminal 2', 'Terminal 3', 'Terminal 4', 'Outside Airport']
...
def print_summary(restaurants: list[Restaurant]) -> None:
    ...
    print(f"\n{divider}")
    print(f"  Singapore Changi Airport \u2014 Restaurants & Dining  ({total} venues)")
...
def main() -> None:
    log.info("Fetching Singapore Changi Airport dining venues from OpenStreetMap \u2026")
